# robomimic — with Conditional Flow Matching

> **This fork adds a Conditional Flow Matching (CFM) policy** — see the section
> [below](#conditional-flow-matching-policy-this-fork) for results, training, and rendering instructions,
> and [CONDITIONAL_FLOW_MATCHING.md](CONDITIONAL_FLOW_MATCHING.md) for the design details.
...
## Conditional Flow Matching policy (this fork)

This fork implements a **Conditional Flow Matching** policy (rectified flow /
OT-CFM; [Lipman et al. 2023](https://arxiv.org/abs/2210.02747),
[Liu et al. 2022](https://arxiv.org/abs/2209.03003)) as a drop-in alternative to
Diffusion Policy. It reuses the same observation encoder and `ConditionalUnet1D`
backbone, but instead of learning to denoise, it learns a **velocity field**
`v(x_t, t, obs)` along the straight-line path from Gaussian noise to action
sequences:

```
x_t = (1 - t)·x0 + t·actions          # linear interpolant, x0 ~ N(0, I)
loss = ‖ v_θ(x_t, t, obs) - (actions - x0) ‖²
```

At test time, actions are generated by integrating the learned ODE with just
**10 Euler steps** (vs. 100 DDPM denoising steps), executed receding-horizon
style exactly like Diffusion Policy.

### Results (low_dim, proficient-human datasets)

| Task | Success rate | Rollouts | Video |
|---|---|---|---|
| Lift | **100%** | 50/50 | [`output/fm_lift_rollout.mp4`](output/fm_lift_rollout.mp4) |
| Tool Hang | **64%** | 32/50 | [`output/fm_toolhang_rollout.mp4`](output/fm_toolhang_rollout.mp4) |

(Tool Hang is the hardest robomimic task — for reference, BC-RNN reaches ~67% and
plain BC ~30% on this dataset.)

### Code map

- [`robomimic/algo/flow_matching.py`](robomimic/algo/flow_matching.py) — the algorithm (`FlowMatchingUNet`, registered as `"flow_matching"`)
- [`robomimic/config/flow_matching_config.py`](robomimic/config/flow_matching_config.py) — config, incl. the `algo.fm` section (`sigma_min`, `num_inference_steps`, `solver`: euler/midpoint, `time_embed_scale`)
- [`robomimic/exps/templates/flow_matching.json`](robomimic/exps/templates/flow_matching.json) — generated config template for `scripts/train.py`
- [`scripts/colab_train_fm.py`](scripts/colab_train_fm.py) — standalone train + eval loop (mirrors the robomimic Get-Started colab)
- [`scripts/render_video.py`](scripts/render_video.py) — rollout video rendering
- [`CONDITIONAL_FLOW_MATCHING.md`](CONDITIONAL_FLOW_MATCHING.md) — design doc

### Quickstart

```bash
# 1. download a dataset (documented robomimic way)
python robomimic/scripts/download_datasets.py --tasks lift --dataset_types ph --hdf5_types low_dim

# 2. train + evaluate (50 rollouts at the end)
python scripts/colab_train_fm.py --dataset <path>/low_dim_v15.hdf5 \
    --num_epochs 1000 --num_rollouts 50 --ckpt output/fm_lift.pth

# 3. render a rollout video (headless: software OSMesa rendering, no X/EGL needed)
MUJOCO_GL=osmesa PYOPENGL_PLATFORM=osmesa \
    python scripts/render_video.py --ckpt output/fm_lift.pth --dataset <path>/low_dim_v15.hdf5
```

The full-featured pipeline also works: `python robomimic/scripts/train.py --config robomimic/exps/templates/flow_matching.json --dataset <path>`.

Note: actions must be normalized to `[-1, 1]` (standard robomimic datasets already are). If your training loop steps schedulers itself, the cosine LR schedule uses `step_every_batch=True` — step it **per gradient step**, or the LR stays at 0 (`scripts/colab_train_fm.py` handles this).
